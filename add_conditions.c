...
void all_regions_max_one_side_product(CNF* formula, unsigned num_of_regions, unsigned num_of_products) {
    ...
    // Zde doplňte řešení
    // Jedna sa o rovnaku funkciu ako max one main product teda jediny rozdiel je pri pridavani
    // literalov do clause. Nepridame MAIN_PROD. teda (T) ale pridame SIDE_PROD. teda (F)
    // Cast klauzuly: ({!v(0,0)||!v(0,1)}&&{!v(0,0)||!v(0,2)})...
    for (unsigned k = 0; k < num_of_regions; ++k) {
        for (unsigned p_1 = 0; p_1 < num_of_products; ++p_1) {
            for (unsigned p_2 = 0; p_2 < num_of_products; ++p_2) {
                if (p_1 >= p_2) {
...
void main_side_products_different(CNF* formula, unsigned num_of_regions, unsigned num_of_products) {
    ...
    // Zde doplňte řešení
    // Pre kazdy kraj skontrolujeme aby nemal rovnaky h(0,p) a v(0,p)
    // teda ({!h(0,0) || !v(0,0)} && {!h(0,1) || !v(0,1)})
    for (unsigned k = 0; k < num_of_regions; ++k) {
        for (unsigned p_1 = 0; p_1 < num_of_products; ++p_1) {
            Clause* cl = create_new_clause(formula);
            add_literal_to_clause(cl, false, MAIN_PRODUCT, k, p_1);
...
void neighbour_regions_different_main_products(CNF* formula, unsigned num_of_regions, unsigned num_of_products, const NeighbourLists* neighbours) {
    ...
    // Příklad použití:
    // unsigned region1 = 0; (r_1)
    // unsigned region2 = 1; (r_2)
    // if (are_neighbours(neighbours, region1, region2)) { ... }

    // Prechadzam jednotlive regiony teda prvy for pre konkretny reg.
    // druhy for pre jeho susedov, def podmienky, nasledne vsetky 
    // main produkty porovnam s ostatnymi main produktami ci sa lisia
    // (bavime sa o susedoch)... ({!h(0,0) || !h(k+1,0)} && {!h(0,0) || !h(k+2,0)}...)
    for (int r_1 = 0; r_1 < num_of_regions; ++r_1) {
        for (int r_2 = 0; r_2 < num_of_regions; ++r_2) {
            if (are_neighbours(neighbours, r_1, r_2)) {
                for (unsigned p = 0; p < num_of_products; ++p) {
                    Clause* cl = create_new_clause(formula);
                    add_literal_to_clause(cl, false, MAIN_PRODUCT, r_1, p);
                    add_literal_to_clause(cl, false, MAIN_PRODUCT, r_2, p);
                }
            }
        }
    }
...
void main_region_main_product_as_side_product_elsewhere(CNF* formula, unsigned num_of_regions, unsigned num_of_products) {
    ...
    // Zde doplňte řešení
    // Main product musi byt vyrabany este niekde inde, teda
    // pomocou vyrokvoej log.: ({!h(0,p) || v(k+1,p)}).
    // Porovnavame hlavny reg., jeho hl. produkt s ostatnymi
    // krajmi-vedlajsimi produktami
    for (int p = 0; p < num_of_products; ++p) {
        Clause* cl = create_new_clause(formula);
        add_literal_to_clause(cl, false, MAIN_PRODUCT, 0, p);
        for (int k = 1; k < num_of_regions; ++k) {
...
char* bonus(void) {
    ...
    // Obdobie v ktorom sa dej odohrava, taktiez tamojsi kral... mozno
    // hlupost ale skusil som :D
    return "kazisvet";
}
